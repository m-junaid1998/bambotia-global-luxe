      ...
      <main className="pt-24 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-10">
            <p className="text-xs tracking-[0.3em] text-accent mb-3">SECURE CHECKOUT</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <section className="bg-background/90 backdrop-blur-md border border-border rounded-2xl p-6 md:p-8 shadow-sm">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="text-[10px] tracking-[0.2em] text-accent">STEP 1</span>
                </div>
              ...
              <section className="bg-background/90 backdrop-blur-md border border-border rounded-2xl p-6 md:p-8 shadow-sm">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="text-[10px] tracking-[0.2em] text-accent">STEP 2</span>
                </div>
                ...
                <div className="border border-accent/40 bg-gold-subtle rounded-xl p-5 flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-accent mt-2">
                      Free shipping on orders over PKR {FREE_SHIPPING_THRESHOLD.toLocaleString()}
                    </p>
                  </div>
                </div>
              </section>
              ...
              <section className="bg-background/90 backdrop-blur-md border border-border rounded-2xl p-6 md:p-8 shadow-sm">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="text-[10px] tracking-[0.2em] text-accent">STEP 3</span>
                </div>

                <label className="flex items-start gap-4 p-5 border-2 border-accent rounded-xl bg-gold-subtle cursor-pointer">
                  <input type="radio" name="payment" defaultChecked className="mt-1 accent-[hsl(var(--accent))]" />
                  <div className="w-11 h-11 rounded-full bg-accent/15 flex items-center justify-center shrink-0">
              ...
              <div className="bg-background/90 backdrop-blur-md border border-border rounded-2xl p-6 md:p-7 shadow-sm">
                ...
                {items.length === 0 ? (
                  <p className="text-sm text-muted-foreground py-6 text-center">
                    Your bag is empty.{" "}
                    <Link to="/" className="text-accent hover:underline">Continue shopping</Link>
                  </p>
                ) : (
                  <>
                        ...
                        <div key={item.productId} className="flex gap-3">
                          <div className="relative shrink-0">
                            <img
                              src={item.image}
                              alt={item.name}
                              className="w-16 h-20 object-cover rounded-md bg-muted border border-border"
                            />
                    ...
                    <div className="space-y-2.5 text-sm">
                      <Row label={`Subtotal (${totalItems} items)`} value={`PKR ${totalPrice.toLocaleString()}`} />
                      <Row
                        label="Delivery Charges"
                        value={shipping === 0 ? "FREE" : `PKR ${shipping.toLocaleString()}`}
                        accent={shipping === 0}
                      />
                    ...
                    <div className="flex justify-between items-baseline">
                      <span className="text-sm tracking-[0.15em] font-medium">TOTAL</span>
                    ...
                    <div className="mt-5 grid grid-cols-2 gap-2 text-[11px] text-muted-foreground">
                      <div className="flex items-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5 text-accent" />Secure Order</div>
                      <div className="flex items-center gap-1.5"><Lock className="w-3.5 h-3.5 text-accent" />Private &amp; Encrypted</div>
                      <div className="flex items-center gap-1.5"><Truck className="w-3.5 h-3.5 text-accent" />Fast Delivery</div>
                      <div className="flex items-center gap-1.5"><Banknote className="w-3.5 h-3.5 text-accent" />Pay on Arrival</div>
                    </div>
                  </>
                )}
              </div>
...
const Row = ({
  ...
  <div className="flex justify-between">
    ...
    <span className={accent ? "text-accent font-semibold" : muted ? "text-muted-foreground" : "text-foreground"}>
      {value}
    </span>
  </div>
);
